...
bool RenderWindow::Initialize(WindowContainer* pWindowContainer, HINSTANCE hInstance, std::string window_title, std::string window_class, int width, int height) {
	...
	//most functions return HRESULT to check what the error was
	//in case of createwindow, it returns handle
...
bool RenderWindow::ProcessMessages()
{
	...
	if (msg.message == WM_NULL)
	{
		if (!IsWindow(this->handle))
		{
			...
			return false;//will return false when window is destroyed
		}
	}
...
LRESULT CALLBACK HandleMsgSetup(HWND hwnd, UINT uMsg, WPARAM wParam, LPARAM lParam) {
	switch (uMsg) {
	//case WM_CHAR: {//only capture characters
	//	unsigned char letter = static_cast<unsigned char>(wParam);
	//	return 0; 
	//}
	//case WM_KEYDOWN: {//catch down controls
